...
# ฟังก์ชันสำหรับคำนวณเวกเตอร์ค่าเฉลี่ย (Mean Vector)
def calculate_mean(data_subset):
    return np.mean(data_subset, axis=0)

# ฟังก์ชันสำหรับคำนวณเมทริกซ์ความแปรปรวน (Covariance Matrix)
...
# ฟังก์ชันสำหรับจำแนกคลาสของตัวอย่างโดยใช้กฎ Bayes Decision
def classify(x, mean1, mean2, cov1, cov2):
    ...
    p1 = dist1.pdf(x)  # คำนวณความน่าจะเป็นของ Class 1
    p2 = dist2.pdf(x)  # คำนวณความน่าจะเป็นของ Class 2
    if p1 > p2:
        return 1  # กลับค่า 1 หาก Class 1 มีความน่าจะเป็นสูงกว่า
    elif p1 < p2:
...
def test_model(class1, class2, features, feature_desc):
    ...
    # ทำการทดสอบแบบ Cross-Validation 10% และทดสอบเต็มรูปแบบ
    accuracies = []
    for test_id in range(1, 12):
        ...
        # แสดงเมทริกซ์ความแปรปรวนในตารางที่จัดรูปแบบ
        cov1_table = [["Cov w1"] + [f"{cov1[i,j]:.4f}" for j in range(n_features) for i in range(n_features)]]
        cov2_table = [["Cov w2"] + [f"{cov2[i,j]:.4f}" for j in range(n_features) for i in range(n_features)]]
...
# ฟังก์ชันสำหรับจัดการการทดลองเพิ่มเติม
def interactive_test(class1, class2):
    while True:
        # ถามผู้ใช้ว่าต้องการทดลองต่อหรือไม่
        continue_test = input("\nต้องการทดลองต่อหรือไม่ ? (yes/no): ").lower()
        if continue_test != 'yes':
            print("สิ้นสุดการทดลอง ขอบคุณที่ใช้งาน!")
            break

        # ถามจำนวนฟีเจอร์ที่ต้องการใช้
        while True:
            try:
                n_features = int(input("กรุณาระบุจำนวนฟีเจอร์ที่ต้องการใช้ (1-4): "))
                if 1 <= n_features <= 4:
                    break
                else:
                    print("กรุณาเลือกจำนวนฟีเจอร์ระหว่าง 1 ถึง 4")
            except ValueError:
                print("กรุณากรอกตัวเลขเท่านั้น")

        # ถามฟีเจอร์ที่ต้องการใช้
        features = []
        for i in range(n_features):
            while True:
                try:
                    feat = int(input(f"กรุณาระบุฟีเจอร์ที่ {i+1} (0-3): "))  # ดัชนี 0-3 หมายถึง Feature 1-4
                    if 0 <= feat <= 3 and feat not in features:
                        features.append(feat)
                        break
                    else:
                        print("ฟีเจอร์นี้ซ้ำหรืออยู่นอกช่วง 0-3 กรุณาลองใหม่")
                except ValueError:
                    print("กรุณากรอกตัวเลขเท่านั้น")

        # สร้างคำอธิบายฟีเจอร์
        feature_desc = f"{n_features} Features ({', '.join([f'Feature {f+1}' for f in features])})"

        # เรียกใช้ฟังก์ชันทดสอบกับฟีเจอร์ที่เลือก
        test_model(class1, class2, features, feature_desc)
...
# เรียกใช้การทดลองแบบโต้ตอบหลังจากรันเริ่มต้น
interactive_test(class1, class2)